...
const requireViewer = vi.fn(
	(user: { id: string } | null | undefined) => user ?? { id: 'viewer-test' }
);
const requireOperator = vi.fn(
	(user: { id: string } | null | undefined) => user ?? { id: 'operator-test' }
);
...
describe('registry API routes', () => {
	...
	it('lists registry data for the requested agent', async () => {
		...
		expect(dispatchRegistryCommand).toHaveBeenCalledWith('agent-1', {
			operation: 'list',
			hive: 'HKEY_CURRENT_USER',
			depth: 2
		});
