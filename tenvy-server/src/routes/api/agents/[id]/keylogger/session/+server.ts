...
function parseConfig(input: Record<string, unknown> | undefined | null): KeyloggerStartConfig {
        const mode = typeof input?.mode === 'string' && input.mode.trim().toLowerCase() === 'offline' ? 'offline' : 'standard';
...
export const POST: RequestHandler = async ({ params, request, locals }) => {
        ...
        const session = keyloggerManager.createSession(id, parseConfig(configInput),
...
export const DELETE: RequestHandler = async ({ params, request, locals }) => {
        ...
        const stopped = keyloggerManager.stopSession(id, typeof body.sessionId === 'string' ? body.sessionId : undefined);
