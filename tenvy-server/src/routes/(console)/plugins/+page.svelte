    ...
    <CardHeader class="space-y-4">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div class="relative w-full max-w-sm">
          ...
          <Input type="search" placeholder="Search plugins, capabilities, or authors" class="pl-10" bind:value={searchTerm} />
        </div>
        <div class="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
          ...
          {#if filtersActive}
            <Button type="button" variant="ghost" size="sm" class="gap-2" on:click={resetFilters}>
      ...
      <div class="grid gap-4 xl:grid-cols-3">
        ...
        <div class="space-y-2">
          <p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Auto updates</p>
          <div class="flex items-center gap-3 rounded-md border border-border/60 px-3 py-2">
            ...
            <div class="min-w-0">
              <p class="text-sm font-medium leading-tight">Only show auto-managed</p>
              <p class="text-xs leading-tight text-muted-foreground">Limit results to plugins with automatic updates enabled.</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <CardHeader class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div class="space-y-2">
              <div class="flex flex-wrap items-center gap-3">
                <CardTitle class="text-base font-semibold leading-tight">{plugin.name}</CardTitle>
              ...
              <CardDescription class="max-w-2xl text-sm text-muted-foreground">{plugin.description}</CardDescription>
            ...
            <div class="flex flex-col gap-4 text-sm text-muted-foreground">
              <div class="flex items-center gap-2">
                <InfoIcon class={cn("h-4 w-4", statusSeverity(plugin.status))} />
          ...
          <CardContent class="grid gap-4 lg:grid-cols-2">
            <div class="flex flex-wrap gap-4 text-sm text-muted-foreground">
              <div class="rounded-md border border-border/60 px-3 py-2">
                <span class="text-xs uppercase tracking-wide">Installations</span>
                <p class="text-lg font-semibold text-foreground">{plugin.installations}</p>
              </div>
              <div class="rounded-md border border-border/60 px-3 py-2">
                <span class="text-xs uppercase tracking-wide">Package size</span>
            ...
            <div class="flex flex-col gap-3">
              <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
                <div class="space-y-1">
                  <p class="text-sm font-medium leading-tight">Plugin enabled</p>
                  <p class="text-xs leading-tight text-muted-foreground">Controls whether the module can run on assigned clients.</p>
                </div>
                <Switch
                  ...
                  onCheckedChange={(value) => {
                    const nextStatus = value
                      ? plugin.status === "disabled" ? "active" : plugin.status
                      : "disabled";
              ...
              <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
                <div class="space-y-1">
                  <p class="text-sm font-medium leading-tight">Automatic updates</p>
                  <p class="text-xs leading-tight text-muted-foreground">When enabled, new builds roll out without manual approval.</p>
                </div>
                <Switch
  ...
  {:else}
    <Card class="border-border/60">
      <CardHeader>
        ...
        <CardDescription>Adjust filters or clear the search query to see registered modules again.</CardDescription>
      </CardHeader>
      <CardFooter>
        <Button type="button" on:click={resetFilters} class="gap-2">
          <RefreshCcwIcon class="h-4 w-4" />
          Reset filters
        </Button>
      </CardFooter>
