  ...
  {#each statusSummaryOrder as status}
    <Card class="border-border/60">
      ...
      <CardContent class="space-y-1">
        ...
        <Badge variant="outline" class={cn("w-fit px-2.5 py-1 text-xs font-medium", statusStyles[status])}>
          {statusLabels[status]}
        </Badge>
      </CardContent>
    </Card>
    ...
    <CardHeader class="space-y-4">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex items-center gap-2">
          <Button type="button" size="sm" variant={viewMode === "table" ? "default" : "outline"} on:click={() => (viewMode = "table")}>
            <LayoutGridIcon class="mr-2 h-4 w-4" />
            Table
          </Button>
          <Button type="button" size="sm" variant={viewMode === "grid" ? "default" : "outline"} on:click={() => (viewMode = "grid")}>
      ...
      <div class="grid gap-4 lg:grid-cols-3">
        ...
        <div class="space-y-2">
          <p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Platform</p>
          <div class="flex flex-wrap gap-2">
            {#each platformFilters as option}
              <Button
                ...
                on:click={() => (platformFilter = option.value)}
              >
                {option.label}
              </Button>
            {/each}
              ...
              <tbody class="divide-y divide-border/60 bg-background">
                {#each filteredClients as client}
                  <tr class="hover:bg-muted/30">
                    ...
                    <td class="px-4 py-3 align-top">
                      <Badge variant="outline" class={cn("px-2.5 py-1 text-xs font-medium", statusStyles[client.status])}>
                    ...
                    <td class="px-4 py-3 align-top">
                      <div class="flex flex-wrap gap-1.5">
                        {#each client.tags as tag}
                          <Badge variant="outline" class="border-border/70 px-2 py-0.5 text-xs font-medium">
                            {tag}
                          </Badge>
                        {/each}
                      </div>
                    ...
                    <td class="px-4 py-3 align-top">
                      <Badge variant="outline" class={cn("px-2.5 py-1 text-xs font-semibold", riskStyles[client.risk])}>
                        {client.risk}
                      </Badge>
                      {#if client.notes}
  ...
  {:else}
    <Card class="border-dashed border-border/60 bg-muted/5">
      <CardContent class="flex flex-col items-center justify-center gap-3 py-16 text-center">
        <UsersIcon class="h-10 w-10 text-muted-foreground/40" />
        <div class="space-y-1">
