...
<section class="space-y-6">
    ...
    <Card class="border border-amber-500/40 bg-amber-500/5">
      <CardHeader class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-3">
          <Badge variant="outline" class="border-amber-500/40 bg-amber-500/10 text-amber-600">Unsaved changes</Badge>
          <p class="text-sm text-muted-foreground">Save updates to propagate them to all operator consoles.</p>
        </div>
      ...
      <CardFooter class="flex flex-wrap items-center justify-end gap-2">
        <Button type="button" variant="outline" size="sm" class="gap-2" on:click={restoreDefaults}>
          <AlertTriangleIcon class="h-4 w-4" />
          Restore defaults
        </Button>
        <Button type="button" size="sm" class="gap-2" on:click={saveChanges}>
  ...
  <Card class="border-border/60">
    ...
    <CardContent class="space-y-6">
      <div class="grid gap-4 md:grid-cols-2">
        <div class="space-y-2">
          ...
          <p class="text-xs text-muted-foreground">Displayed to invited operators and in automation signatures.</p>
        </div>
        <div class="space-y-2">
          <Label for="control-plane-host">Control plane host</Label>
          <Input id="control-plane-host" bind:value={generalControlPlaneHost} />
          <p class="text-xs text-muted-foreground">Primary endpoint used for clients to establish uplinks.</p>
        </div>
        <div class="space-y-2">
          <Label for="maintenance-window">Maintenance window</Label>
          <Input id="maintenance-window" bind:value={generalMaintenanceWindow} />
          <p class="text-xs text-muted-foreground">Communicated to operators before scheduled downtime.</p>
        </div>
      </div>
      <Separator />
      <div class="grid gap-4 md:grid-cols-2">
        <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
          <div class="space-y-1">
            <p class="text-sm font-medium leading-tight">Automatic controller updates</p>
            <p class="text-xs leading-tight text-muted-foreground">Applies security hotfixes without manual approval.</p>
          </div>
          <Switch bind:checked={generalAutoUpdate} aria-label="Toggle automatic updates" />
        </div>
        <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
          <div class="space-y-1">
            <p class="text-sm font-medium leading-tight">Allow beta channels</p>
            <p class="text-xs leading-tight text-muted-foreground">Opt-in to preview builds for early capability access.</p>
          </div>
    ...
    <CardFooter class="flex flex-wrap items-center justify-between gap-3">
      <p class="text-xs text-muted-foreground">Changes replicate to all connected consoles after saving.</p>
      <Button type="button" variant="outline" size="sm" on:click={() => resetSection("general")} disabled={!generalDirty}>
        Revert section
      </Button>
    </CardFooter>
  </Card>
  ...
  <Card class="border-border/60">
    ...
    <CardContent class="space-y-6">
      <div class="space-y-4">
        <p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Digest frequency</p>
        <div class="flex flex-wrap gap-2">
          {#each digestOptions as option}
            <Button
              ...
              on:click={() => (notificationsDigestFrequency = option.value)}
            >
              {option.label}
            </Button>
          {/each}
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
          <div class="space-y-1">
            <p class="text-sm font-medium leading-tight">Real-time operations alerts</p>
            <p class="text-xs leading-tight text-muted-foreground">Send notifications for new tasks, pivots, and command results.</p>
          </div>
          <Switch bind:checked={notificationsRealtimeOps} aria-label="Toggle real-time alerts" />
        </div>
        <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
          <div class="space-y-1">
            <p class="text-sm font-medium leading-tight">Escalate critical alerts</p>
            <p class="text-xs leading-tight text-muted-foreground">Escalate high-risk detections to the incident bridge.</p>
          </div>
          <Switch bind:checked={notificationsEscalateCritical} aria-label="Toggle critical escalations" />
        </div>
        <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
          <div class="space-y-1">
            <p class="text-sm font-medium leading-tight">Email bridge</p>
            <p class="text-xs leading-tight text-muted-foreground">Deliver summaries to the operations distribution list.</p>
          </div>
          <Switch bind:checked={notificationsEmailBridge} aria-label="Toggle email bridge" />
        </div>
        <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
          <div class="space-y-1">
            <p class="text-sm font-medium leading-tight">Slack bridge</p>
            <p class="text-xs leading-tight text-muted-foreground">Mirror alerts to the #ops-war-room channel.</p>
          </div>
    ...
    <CardFooter class="flex flex-wrap items-center justify-between gap-3">
      <p class="text-xs text-muted-foreground">Adjust channel mix to keep operators aligned with mission tempo.</p>
      <Button
        type="button"
        variant="outline"
        size="sm"
        on:click={() => resetSection("notifications")}
  ...
  <Card class="border-border/60">
    ...
    <CardContent class="space-y-6">
      <div class="grid gap-4 md:grid-cols-2">
        <div class="space-y-2">
          ...
          <Input id="session-timeout" type="number" min="5" bind:value={securitySessionTimeoutMinutes} />
          <p class="text-xs text-muted-foreground">Force operators to re-authenticate after periods of inactivity.</p>
        </div>
        <div class="space-y-2">
          ...
          <p class="text-xs text-muted-foreground">Minimum approvers required for destructive queued tasks.</p>
        </div>
      </div>
      <Separator />
      <div class="grid gap-4 md:grid-cols-2">
        <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
          <div class="space-y-1">
            <p class="text-sm font-medium leading-tight">Require multi-factor authentication</p>
            <p class="text-xs leading-tight text-muted-foreground">Enforce MFA on every operator login.</p>
          </div>
          <Switch bind:checked={securityEnforceMfa} aria-label="Toggle MFA requirement" />
        </div>
        <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
          <div class="space-y-1">
            <p class="text-sm font-medium leading-tight">IP allowlist enforcement</p>
            <p class="text-xs leading-tight text-muted-foreground">Restrict console logins to the trusted operations network.</p>
          </div>
          <Switch bind:checked={securityIpAllowlist} aria-label="Toggle IP allowlist" />
        </div>
        <div class="flex items-center justify-between rounded-md border border-border/60 px-3 py-2">
          <div class="space-y-1">
            <p class="text-sm font-medium leading-tight">Require runbook approval</p>
            <p class="text-xs leading-tight text-muted-foreground">Route high-impact automations through approval workflow.</p>
          </div>
    ...
    <CardFooter class="flex flex-wrap items-center justify-between gap-3">
      ...
      <Button type="button" variant="outline" size="sm" on:click={() => resetSection("security")} disabled={!securityDirty}>
        Revert section
      </Button>
    </CardFooter>
  </Card>

  <div class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border/60 bg-muted/30 px-4 py-3 text-sm">
    <div class="flex items-center gap-3 text-muted-foreground">
      <UserPlusIcon class="h-4 w-4" />
      Invite additional operators once security policies are confirmed.
    </div>
    <div class="flex items-center gap-2">
      <Button type="button" variant="ghost" size="sm" class="gap-2" on:click={restoreDefaults}>
        <AlertTriangleIcon class="h-4 w-4" />
        Restore defaults
      </Button>
      <Button type="button" size="sm" class="gap-2" on:click={saveChanges} disabled={!hasChanges}>
        <CheckIcon class="h-4 w-4" />
        Save changes
      </Button>
    </div>
