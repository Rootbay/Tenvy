		...
		<CardContent class="space-y-4">
			<div class="grid gap-4 md:grid-cols-2">
				<div class="grid gap-2">
					...
					<Input id="options-beacon" type="number" min={5} step={5} bind:value={beaconInterval} />
				</div>
				<div class="grid gap-2">
					<Label for="options-jitter">Jitter (%)</Label>
					<Input id="options-jitter" type="number" min={0} max={100} bind:value={jitterPercent} />
				</div>
			</div>
		</CardContent>
	</Card>
			...
			>
				<div>
					...
					<p class="text-xs text-muted-foreground">
						Only operate when controller is on VPN subnets
					</p>
				</div>
