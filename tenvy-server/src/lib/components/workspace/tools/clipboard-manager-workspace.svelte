<script lang="ts">
        ...
        function describePlan(): string {
                const segments = [`${captureText ? 'text' : ''} ${captureImages ? 'image' : ''} ${captureFiles ? 'files' : ''}`.trim(), `interval ${pollInterval}s`, redactSecrets ? 'redaction on' : 'redaction off'];
