<script lang="ts">
        ...
        const {
                client,
                surface = 'workspace'
        } = $props<{ client: Client; surface?: 'workspace' | 'dialog' }>();
...
<div class={`flex h-full flex-col ${surface === 'workspace' ? 'gap-6' : ''}`}>
        {#if surface === 'workspace'}
                <WorkspaceHeroHeader {client} tool={tool} metadata={heroMetadata} />
        {/if}
                                        ...
                                        <Card class="border-border/60">
                                                ...
                                                <CardContent class="grid gap-4 text-sm sm:grid-cols-2">
                                                        ...
                                                        <div class="sm:col-span-2">
                                                                ...
                                                                <p class="mt-1 font-medium text-foreground">
                                                                        PID {formatNumber(report.runtime.process?.pid)} · {report.runtime.process?.commandLine ?? '—'}
                                                                </p>
                                                                <p class="text-xs text-muted-foreground">
