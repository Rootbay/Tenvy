...
import type { ProcessDetail, ProcessStatus, ProcessSummary } from '$lib/types/task-manager';
...
export function toDetail(process: ProcessInfo): ProcessDetail {
	...
	const args =
		typeof process.params === 'string' && process.params.trim() !== ''
			? splitCommandLine(process.params)
			: [];
	const cpuTime =
		Number.isFinite(process.cpuu) || Number.isFinite(process.cpus)
			? (Number.isFinite(process.cpuu) ? process.cpuu : 0) +
				(Number.isFinite(process.cpus) ? process.cpus : 0)
			: undefined;
